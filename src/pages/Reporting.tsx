...
export function Reporting({ cars, clients, onBack }: ReportingProps) {
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto animate-in fade-in duration-500 text-left">
      <div className="mb-6">
        <h1 className="text-3xl font-black text-black uppercase tracking-tight italic leading-none">Отчетность</h1>
      </div>
      ...
      <div className="p-8 bg-black text-white rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-8 mb-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex gap-12 relative z-10 font-sans">
          <div><p className="text-[10px] text-slate-400 uppercase font-black mb-1 tracking-widest">Итоговая сумма</p><p className="text-3xl font-black whitespace-nowrap">{totals.sale.toLocaleString()} ₽</p></div>
          <div><p className="text-[10px] text-slate-400 uppercase font-black mb-1 tracking-widest">Закупка</p><p className="text-3xl font-black text-slate-600 whitespace-nowrap">{totals.purchase.toLocaleString()} ₽</p></div>
        </div>
        <div className="text-right border-l border-white/10 pl-12 relative z-10 font-sans">
          <p className="text-[10px] text-slate-400 uppercase font-black mb-1 tracking-widest">Чистая прибыль</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex gap-4 items-end">
          {reportMode === 'month' ? (
            <>
              <div className="flex-1"><label className="text-[10px] font-black uppercase text-slate-400 mb-1.5 block ml-1 font-sans tracking-widest">Месяц</label><CustomSelect options={months} value={months[selectedMonth]} onChange={(val: string) => setSelectedMonth(months.indexOf(val))} /></div>
              <div className="w-28"><label className="text-[10px] font-black uppercase text-slate-400 mb-1.5 block ml-1 font-sans tracking-widest">Год</label><CustomSelect options={years} value={selectedYear.toString()} onChange={(val: string) => setSelectedYear(Number(val))} /></div>
            </>
          ) : (
            <>
              <div className="flex-1"><label className="text-[10px] font-black uppercase text-slate-400 mb-1.5 block ml-1 font-sans tracking-widest">От</label><input type="date" className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl outline-none font-bold text-xs shadow-sm focus:border-green-500 transition-all font-sans" value={customRange.start} onChange={e => setCustomRange({...customRange, start: e.target.value})} /></div>
              <div className="flex-1"><label className="text-[10px] font-black uppercase text-slate-400 mb-1.5 block ml-1 font-sans tracking-widest">До</label><input type="date" className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl outline-none font-bold text-xs shadow-sm focus:border-green-500 transition-all font-sans" value={customRange.end} onChange={e => setCustomRange({...customRange, end: e.target.value})} /></div>
            </>
          )}
          <button onClick={() => setReportMode(reportMode === 'month' ? 'custom' : 'month')} className="bg-slate-100 text-slate-500 px-3 h-10 rounded-xl font-black text-[9px] uppercase border border-slate-200 hover:bg-slate-200 font-sans tracking-tight">{reportMode === 'month' ? 'Период' : 'Месяц'}</button>
        </div>
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex gap-3 items-end">
          <div className="flex-1"><label className="text-[10px] font-black uppercase text-slate-400 mb-1.5 block ml-1 font-sans tracking-widest">Клиент</label><CustomSelect options={['Все клиенты', ...clients.map((c:any) => c.fullName)]} value={selectedClientName} onChange={(val: string) => { if (val === 'Все клиенты') setFilterClientId('all'); else setFilterClientId(clients.find((c:any) => c.fullName === val)?.id || 'all'); setFilterCarId('all'); }} /></div>
          <div className="flex-1"><label className="text-[10px] font-black uppercase text-slate-400 mb-1.5 block ml-1 font-sans tracking-widest">Машина</label><CustomSelect className={filterClientId === 'all' ? 'opacity-30 pointer-events-none' : ''} options={['Все авто', ...availableCars.map((c:any) => `${c.brand} ${c.model}`)]} value={selectedCarName} onChange={(val: string) => { if (val === 'Все авто') setFilterCarId('all'); else setFilterCarId(availableCars.find((c:any) => `${c.brand} ${c.model}` === val)?.id || 'all'); }} /></div>
          <button onClick={exportPDF} className="btn-action !h-10 !px-4 hover:bg-green-600 font-sans"><Download size={16} /></button>
        </div>
      </div>

      <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden">
        <div className="overflow-auto max-h-[calc(100vh-250px)]">
          <table className="w-full text-left border-collapse min-w-[1000px]" style={timesNewRoman}>
            <thead className="sticky top-0 bg-slate-50 z-10 border-b border-slate-200 shadow-sm text-slate-500 text-[11px] font-bold uppercase tracking-wider font-sans">
              <tr>
            ...
            <tbody className="divide-y divide-slate-50 text-[14px]">
              {filteredData.map((r, i) => (
                <tr key={i} className="hover:bg-green-50 transition-colors">
                  <td className="px-6 py-3 text-slate-400 font-bold">{format(parseISO(r.date), 'dd.MM.yyyy')}</td>
                  <td className="px-6 py-3 leading-tight"><div className="font-bold text-slate-950 uppercase">{r.clientName}</div><div className="text-[11px] font-bold text-slate-400 font-sans tracking-tight">{r.carName}</div></td>
                  ...
                  <td className="px-6 py-3 text-right font-bold text-black whitespace-nowrap">{r.totalPrice.toLocaleString()} ₽</td>
