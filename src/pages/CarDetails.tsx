...
export function CarDetails({ carId, clients, cars, noteOptions, addRecord, updateRecord, deleteRecord, onBack }: any) {
  ...
  const filteredGroupedRecords = useMemo(() => {
    ...
    return Object.keys(groups).sort((a,b) => b.localeCompare(a)).map(date => ({
      ...
      // Итоги считаются только для статуса 2 (Выполнено)
      sale: groups[date].reduce((s: number, r: any) => s + (r.status === 2 ? r.totalPrice : 0), 0),
      purchase: groups[date].reduce((s: number, r: any) => s + (r.status === 2 ? r.purchasePrice : 0), 0)
    }));
  }, [car, historySearch]);
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 p-8 mb-6 flex justify-between items-start shadow-sm relative overflow-hidden">
        ...
        <div className="flex gap-10 items-center h-full self-center text-left">
        ...
        <div className="overflow-auto max-h-[calc(100vh-320px)] min-h-[350px]">
          <table className="w-full text-left table-fixed min-w-[1100px] border-collapse">
            <thead className="sticky top-0 z-20 bg-slate-50 border-b border-slate-200 shadow-sm text-slate-400 text-[10px] font-black uppercase">
              <tr>
                <th className="w-[120px] px-4 py-3 text-center">Статус / Дата</th>
              ...
              {(isAdding || editingId) && (
                <tr className="bg-white border-b-2 border-green-500 sticky top-[42px] z-[50] shadow-[0_4px_15px_rgba(0,0,0,0.1)]">
                  <td className="p-1.5 bg-white">
                    <input type="date" className="w-full p-2 border border-slate-200 rounded-lg text-[10px] font-bold outline-none mb-1" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})}/>
                    <div className="flex items-center gap-1 bg-blue-50 p-1 rounded border border-blue-100">
                      <Wallet size={10} className="text-blue-500" />
                      <input type="number" placeholder="Предоплата" className="w-full bg-transparent text-[10px] font-black text-blue-600 outline-none" value={formData.prepayment} onChange={e => setFormData({...formData, prepayment: e.target.value})} />
                    </div>
                  </td>
                  ...
                  <td className="p-1.5 bg-white"><input type="number" className="w-full p-2 border border-slate-200 rounded-lg text-[11px] text-center font-bold outline-none" value={formData.quantity} onChange={e => setFormData({...formData, quantity: e.target.value})}/></td>
                  ...
                  <td className="p-1.5 bg-white flex gap-1 justify-center items-center h-[70px]">
              ...
              {filteredGroupedRecords.map((g: any) => (
                <React.Fragment key={g.date}>
                  <tr className="bg-slate-50/80 border-y border-slate-100 text-slate-400 text-[10px] font-black uppercase">
                    <td colSpan={9} className="px-4 py-1.5 flex items-center gap-2 tracking-widest whitespace-nowrap"><Calendar size={12} className="text-green-600" />{format(new Date(g.date), 'dd.MM.yyyy')}</td>
                  </tr>
                  {g.records.map((r: any) => (
                    <tr key={r.id} className={`transition-colors group border-b border-slate-50 last:border-0 ${r.status === 1 ? 'bg-yellow-50' : r.status === 2 ? 'bg-green-100' : 'bg-white hover:bg-green-50/50'}`}>
                      <td className="px-4 py-2">
                        <div className="flex gap-1 justify-center">
                          <button onClick={() => updateStatus(r.id, 1)} title="Заказано" className={`p-1.5 rounded-lg ${r.status === 1 ? 'bg-yellow-400 text-white' : 'bg-slate-50 text-slate-300 hover:bg-yellow-200 hover:text-yellow-700'}`}><ShoppingCart size={14} /></button>
                          <button onClick={() => updateStatus(r.id, 2)} title="Выполнено" className={`p-1.5 rounded-lg ${r.status === 2 ? 'bg-green-500 text-white' : 'bg-slate-50 text-slate-300 hover:bg-green-200 hover:text-green-700'}`}><CheckCircle2 size={14} /></button>
                          {r.status > 0 && <button onClick={() => updateStatus(r.id, 0)} title="Сброс" className="p-1.5 bg-slate-50 text-slate-300 rounded-lg hover:bg-slate-200"><RotateCcw size={14} /></button>}
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-1.5 text-right font-black text-black whitespace-nowrap">
                        ...
                        {r.prepayment > 0 && (
                          <div className="text-[10px] text-blue-600 font-black flex items-center justify-end gap-1 mt-0.5" title="Получена предоплата">
                            <Wallet size={10} /> {r.prepayment.toLocaleString()} ₽
                          </div>
                        )}
                      ...
                      <td className="px-3 py-1.5">
                        <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-all">
                          <button onClick={() => { setEditingId(r.id); setFormData({...r, unitPriceSale: r.unitPriceSale || (r.totalPrice/r.quantity), unitPricePurchase: r.unitPricePurchase || (r.purchasePrice/r.quantity)}); }} className="p-1.5 text-slate-300 hover:text-green-600"><Edit2 size={14} /></button>
                          <button onClick={() => deleteRecord(car.id, r.id)} className="p-1.5 text-slate-300 hover:text-red-600"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
                  <tr className="bg-slate-50/40 text-right uppercase border-b border-slate-200/50">
                    <td colSpan={5} className="px-6 py-3 text-slate-400 italic text-[12px] font-black tracking-widest uppercase">Итого (выполнено):</td>
                    ...
                    <td colSpan={2} className="px-6 py-3 text-left pl-10 text-[12px] font-black italic tracking-tight whitespace-nowrap">Прибыль: <span className={g.sale-g.purchase >= 0 ? 'text-green-600 text-[15px]' : 'text-red-600 text-[15px]'}>{(g.sale-g.purchase).toLocaleString()} ₽</span></td>
                  </tr>
                </React.Fragment>
              ))}
            </tbody>
