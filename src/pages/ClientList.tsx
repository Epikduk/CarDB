...
export function ClientList({ 
  ...
  // Функция для безопасного клика (не переходит, если выделяем текст)
  const handleCarClick = (carId: string) => {
    const selection = window.getSelection();
    if (selection && selection.toString().length > 0) {
      // Если есть выделенный текст, отменяем переход, чтобы дать скопировать
      return;
    }
    onSelectCar(carId);
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto animate-in fade-in duration-500 text-left">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-black text-black uppercase italic tracking-tight leading-none font-sans">База клиентов</h1>
        <div className="flex items-center gap-4">
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-sm">
            <button onClick={() => setSortBy('name')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${sortBy === 'name' ? 'bg-white text-black shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><SortAsc size={14} /> А-Я</button>
            <button onClick={() => setSortBy('cars')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${sortBy === 'cars' ? 'bg-white text-black shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><CarFront size={14} /> По авто</button>
          </div>
          <button onClick={() => setModal({ type: 'addClient' })} className="btn-action !py-3 !px-6"><UserPlus size={18} /> <span>Новый клиент</span></button>
        </div>
      </div>

      <div className="relative mb-6">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
        <input type="text" placeholder="Поиск по базе..." className="w-full h-12 pl-12 pr-4 bg-white border border-slate-200 rounded-2xl outline-none focus:border-green-500 shadow-sm transition-all font-bold text-sm font-sans" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {groupedData.map(({ client, clientCars, filteredCars }: any) => {
          ...
          return (
            <div key={client.id} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden hover:border-green-500/20 transition-all font-sans">
              <div onClick={() => toggleExpand(client.id)} className={`p-4 bg-slate-50/50 flex justify-between items-center cursor-pointer group/card relative z-10 transition-all border-b ${isExpanded ? 'border-slate-200' : 'border-transparent'}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-slate-400 text-[11px] font-bold uppercase mt-1.5 leading-none tracking-widest">
                      ...
                      <div className="flex items-center gap-4 flex-1 text-left min-w-0 overflow-hidden pr-4">
                        ...
                        <div className="flex items-center w-full min-w-0">
                          <div className="w-[280px] flex-shrink-0 flex items-center pr-4">
                            <span className="text-[15px] font-bold text-slate-800 group-hover:text-black uppercase truncate">{car.brand} {car.model}</span>
                            <span className="text-slate-400 text-[15px] ml-3 flex-shrink-0 font-normal">{car.year}</span>
                          </div>
