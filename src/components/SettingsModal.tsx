...
export function SettingsModal({ isOpen, onClose, options, onUpdate }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center z-[150] p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.2)] w-full max-w-sm overflow-hidden animate-in zoom-in duration-200 border border-white/20">
        <div className="flex items-center p-5 border-b bg-slate-50/50">
          {activeView !== 'menu' && (
          ...
          <h2 className="text-lg font-black text-slate-950 uppercase italic tracking-tighter">
        ...
        <div className="p-6 min-h-[250px] max-h-[70vh] overflow-y-auto">
          {activeView === 'menu' ? (
            <button onClick={() => setActiveView('notes')} className="w-full flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-emerald-500 hover:shadow-xl transition-all group text-left shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Список для истории</div>
                </div>
              </div>
            ...
            <div className="animate-in slide-in-from-right-4 duration-300">
              <div className="flex gap-2 mb-4">
                <input type="text" autoFocus className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl focus:border-emerald-500 outline-none font-bold text-xs" placeholder="Новый вариант..." value={newOption} onChange={(e) => setNewOption(e.target.value)} onKeyDown={(e) => {if(e.key==='Enter' && newOption.trim()){ onUpdate([...options, newOption.trim()]); setNewOption(''); }}} />
                <button onClick={() => {if(newOption.trim()){ onUpdate([...options, newOption.trim()]); setNewOption(''); }}} className="bg-slate-950 text-white p-2.5 rounded-xl hover:bg-emerald-600 transition-all"><Plus size={20} /></button>
              </div>
              <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                {options.map((opt: string, i: number) => (
                  <div key={i} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100/50 group">
                    <span className="text-slate-700 font-bold text-xs italic">{opt}</span>
