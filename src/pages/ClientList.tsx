...
export function ClientList({ clients, cars, addClient, updateClient, deleteClient, addCarToClient, updateCar, deleteCar, onSelectCar }: any) {
  ...
  const groupedData = useMemo(() => {
    ...
    return clients.map((client: any) => {
      ...
      return { 
        ...
        clientCars, 
        filteredCars, 
        hasMatch: isClientMatch || filteredCars.length > 0 
      };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-black text-black uppercase italic tracking-tight">База клиентов</h1>
      ...
      <div className="grid grid-cols-1 gap-4">
        {groupedData.map(({ client, clientCars, filteredCars }: any) => {
          ...
          return (
            <div key={client.id} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden hover:border-green-500/30 transition-all">
              <div className="p-4 bg-slate-50/50 border-b flex justify-between items-center">
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setModal({ type: 'editClient', data: client })} className="p-1 text-slate-400 hover:text-green-600 transition-colors"><Edit2 size={14}/></button>
                  </div>
                  <div className="flex items-center gap-2 text-slate-500 text-[11px] font-bold uppercase mt-1.5"><Phone size={12} className="text-green-600"/>{client.phone}</div>
                </div>
                <div className="flex items-center gap-3">
